...
/**
 * Landing hero — identify the person, point at the rest, get out of the
 * way. Big serif name, gradient sub-heading, two CTAs, a small
 * "lab readout" strip at the bottom.
 */
    ...
    >
      <div className="relative z-10 max-w-3xl">
        <div className="inline-flex items-center gap-2 font-mono text-[11px] uppercase tracking-[0.22em] text-muted-foreground">
          <span className="status-breath inline-flex h-1.5 w-1.5 rounded-full bg-[var(--brand-emerald)]" />
          kwanho.dev
        </div>
        ...
        <h1 className="mt-7 font-display text-[clamp(2.5rem,7vw,5.25rem)] leading-[1.02] tracking-tight">
          ...
          <span className="bg-gradient-to-r from-[var(--brand-emerald)] via-[var(--brand-cyan)] to-[var(--brand-violet)] bg-clip-text text-transparent">
            Software for robots, vision, and cars.
          </span>
        </h1>

        <p className="mt-7 max-w-2xl text-base md:text-lg leading-8 text-muted-foreground">
          The two demos below run as their own apps under kwanho.dev. The longer
          profile — how I got here, what I think about while shipping — is at{' '}
          <Link href="/editor" className="text-foreground underline decoration-border underline-offset-4 hover:decoration-foreground">
            /editor
          </Link>.
        </p>
      ...
      {/* Bottom strip — small "lab readout" */}
      <dl className="relative z-10 mt-16 grid max-w-2xl grid-cols-1 gap-x-8 gap-y-4 border-t border-border/60 pt-6 font-mono text-[11px] uppercase tracking-[0.16em] text-muted-foreground sm:grid-cols-3">
        <div>
          <dt>Based in</dt>
        ...
        <div>
          <dt>Areas</dt>
          <dd className="mt-1 text-foreground normal-case">Robotics · Vision · Automotive</dd>
        </div>
        <div>
          <dt>Stack</dt>
