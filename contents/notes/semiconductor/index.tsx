  ...
  return (
    <NoteRoot accent="var(--brand-cyan)">
      ...
      <Section id="c2">
        ...
        <Callout label="거꾸로 가는 직관: ">
          금속(도체)은 <B>뜨거워지면 저항이 커져</B> 전기가 덜 통한다 — 원자가 심하게 진동해 전자의 길을 방해하기 때문이다.
          그런데 반도체는 정반대다. 온도가 오르면 열에너지를 받아 밴드갭을 넘는 전자가 늘어 <B>오히려 더 잘 통한다</B>. 바로
          이 "온도에 민감하다"는 성질 때문에 칩은 발열 관리가 생명이고(과열되면 누설이 급증), 동시에 같은 원리로 온도
          센서·서미스터를 만들 수도 있다. 밴드갭은 빛으로도 넘을 수 있어서, 빛을 받으면 전기를 만드는 태양전지와 빛을 감지하는
          이미지 센서(카메라)도 모두 이 한 장의 원리 위에 서 있다.
        </Callout>
      </Section>
      ...
      <Section id="c3">
        ...
        <Deep title="한 걸음 더 · 왜 게르마늄도 GaAs도 아닌 실리콘인가">
          밴드갭만 보면 후보가 여럿이다. 초기엔 <B>게르마늄(Ge)</B>을 썼지만 밴드갭이 너무 작아 열에 약했고(쉽게 새어 흐름),
          결정적으로 <Em>좋은 산화막이 안 생긴다</Em>. <B>갈륨비소(GaAs)</B>는 전자가 더 빨라 고주파엔 유리하지만 비싸고 큰
          웨이퍼를 만들기 어렵다. 실리콘이 이긴 진짜 이유는 성능이 아니라 <B>SiO₂라는 거의 완벽한 천연 절연막</B>이다 — 게이트
          아래 절연층(6장)이 공짜로 깔끔하게 생기니, 트랜지스터를 대량으로 안정되게 찍어낼 수 있다. "가장 좋은 재료"가 아니라
          "<Em>가장 다루기 좋은 재료</Em>"가 표준이 된 셈이다.
        </Deep>
        <P>
          한 가지 더 — 칩에 쓰는 실리콘은 그냥 모래가 아니라 <B>불순물이 10억분의 1 수준</B>인 초고순도 단결정이다. 녹인
          실리콘에 씨앗 결정을 담가 천천히 돌려 뽑아 올리면(초크랄스키 공정) 원자들이 한 방향으로 가지런히 정렬된 거대한
          기둥(잉곳)이 자란다. 이걸 얇게 썰어 거울처럼 연마한 게 <B>웨이퍼</B>다. 결정이 조금만 흐트러져도 회로가 망가지므로,
          "가지런함"과 "깨끗함"이 곧 품질이다(10장).
        </P>
        <Note label="1부 요약: ">
      ...
      <Section id="c4">
        ...
        <Deep title="한 걸음 더 · 다수·소수 캐리어와 도핑의 위력">
          도핑은 양이 핵심이다. 실리콘 원자는 1cm³에 약 <B>5×10²²개</B>인데, 도핑은 보통 그중 <B>10¹⁵~10¹⁹개</B>(즉 백만~천만
          분의 일)만 불순물로 바꾼다. 이 극소량이 전도도를 <Em>수백만 배</Em> 끌어올린다. N형에서는 전자가 압도적으로 많아
          <B>다수 캐리어</B>, 정공은 <B>소수 캐리어</B>가 되고, P형은 그 반대다. 이 "다수가 무엇이냐"가 5장에서 두 조각을 붙일
          때 어떤 일이 벌어지는지를 결정한다. 같은 자리에 인(N)을 진하게 박았다가 붕소(P)를 더 진하게 덮으면 성질을 뒤집을
          수도 있어, 이 농도 조절이 곧 회로 설계의 물리적 실체다.
        </Deep>
        <Deep title="학부 수준 · 전류의 두 메커니즘: 드리프트와 확산">
          캐리어가 전류를 만드는 길은 둘이다. ① <B>드리프트(drift)</B> — 전기장 E가 있으면 캐리어가 휩쓸려 흐른다. 전류밀도{' '}
          <Kbd>J = q·n·μ·E</Kbd>(n=캐리어 농도, μ=이동도). ② <B>확산(diffusion)</B> — 농도가 불균일하면 많은 쪽에서 적은
          쪽으로 퍼진다(Fick 법칙, J ∝ −dn/dx). 둘을 잇는 다리가 <B>아인슈타인 관계</B> <Kbd>D/μ = kT/q</Kbd>다. 이동도 μ는
          전자가 정공보다 약 2~3배 커서 <Em>NMOS가 PMOS보다 빠르다</Em> — CMOS에서 PMOS를 더 넓게 그려 균형을 맞추는 이유다.
          또 도핑을 높이면 캐리어 수 n은 늘지만 불순물 산란이 심해져 μ는 떨어지는 트레이드오프가 있다.
        </Deep>
      ...
      <Section id="c5">
        ...
        <Deep title="학부 수준 · PN 접합을 수식으로">
          경계엔 저절로 전위 장벽이 생긴다 — <B>내부 전위(built-in potential)</B> <Kbd>V_bi = (kT/q)·ln(N_A·N_D / n_i²)</Kbd>
          로, 실리콘에선 대략 0.7V다. 공핍층 폭은 <Kbd>W ∝ √(V_bi − V)</Kbd>라, 역방향 전압을 키우면 공핍층이 넓어진다(이
          가변 정전용량을 이용한 게 버랙터 다이오드). 그리고 그 유명한 <B>다이오드 방정식</B>{' '}
          <Kbd>I = I_s·(exp(qV/kT) − 1)</Kbd> — 순방향에선 전압에 <Em>지수적으로</Em> 폭증하고 역방향에선 −I_s로 막힌다.
          여기 등장하는 <B>열전압 kT/q ≈ 26mV</B>(상온)는 거의 모든 반도체 수식에 나오는 기본 상수다.
        </Deep>
        <Quiz q="공핍층이 하는 일은?">
      ...
      <Section id="c6">
        ...
        <Deep title="학부 수준 · MOSFET I–V 특성">
          게이트 전압이 문턱 V_th를 넘으면(V_GS &gt; V_th) 채널이 열린다. ① <B>선형(트라이오드) 영역</B>(V_DS 작을 때):{' '}
          <Kbd>I_D ≈ μ·C_ox·(W/L)·(V_GS − V_th)·V_DS</Kbd> — 저항처럼 동작. ② <B>포화 영역</B>(V_DS ≥ V_GS − V_th):{' '}
          <Kbd>I_D ≈ ½·μ·C_ox·(W/L)·(V_GS − V_th)²</Kbd> — 전류가 <Em>(V_GS−V_th)의 제곱</Em>에 비례하고 V_DS엔 거의
          무관(정전류원처럼). 여기서 <B>C_ox</B>는 게이트 산화막의 단위면적 정전용량, <B>W/L</B>은 채널 폭/길이 비다. 산화막을
          얇게(C_ox↑) 하거나 채널을 넓게(W↑) 하면 같은 전압에 더 센 전류가 흐른다. 디지털 스위치는 차단(OFF)↔포화(ON)를
          오가고, <B>트랜스컨덕턴스 gm = ∂I_D/∂V_GS</B>가 "게이트가 전류를 얼마나 세게 조이나"를 나타낸다.
        </Deep>
      ...
      <Section id="c7">
        ...
        <Deep title="한 걸음 더 · NOT 게이트 하나로 보는 CMOS의 마법">
          가장 단순한 CMOS 회로인 인버터(NOT)는 PMOS 하나와 NMOS 하나를 위아래로 쌓은 것이다. 입력이{' '}
          <Kbd>0</Kbd>(낮음)이면 위쪽 PMOS만 켜져 출력을 전원(+)에 연결 → 출력 <Kbd>1</Kbd>. 입력이 <Kbd>1</Kbd>이면 아래쪽
          NMOS만 켜져 출력을 접지(0)에 연결 → 출력 <Kbd>0</Kbd>. 핵심은 <B>두 트랜지스터가 동시에 켜지는 일이 (거의) 없다</B>
          는 것 — 그래서 전원에서 접지로 곧장 새는 길이 없다. 전류는 0↔1로 <Em>바뀌는 찰나</Em>에만 잠깐 흐른다(스위칭 전력).
          그래서 칩의 동적 전력은 대략 "<B>스위칭 횟수 × 클럭</B>"에 비례하고, 가만히 있을 땐 거의 0이다. 이 우아함 덕분에
          손톱만 한 칩에 수백억 개를 넣고도 녹지 않는다(가만히 있어도 새는 누설 전력 이야기는 14·17장).
        </Deep>
        <Deep title="학부 수준 · CMOS의 전력·지연·잡음 여유">
          <B>동적 전력</B> <Kbd>P_dyn = α·C·V_DD²·f</Kbd> (α=스위칭 활동률, C=부하 용량, f=클럭). 전압의{' '}
          <Em>제곱</Em>에 비례하므로 V_DD를 낮추는 게 전력 절감의 왕도였다(그러나 V_th 때문에 한계, 9·14장). <B>전파 지연</B>{' '}
          <Kbd>t_pd ∝ C·V_DD / I_on</Kbd> — 더 센 구동 전류(I_on)와 더 작은 용량(C)이 빠른 칩을 만든다. 또 CMOS는 출력이
          전원이나 접지에 딱 붙어 흔들리지 않아 <B>잡음 여유(noise margin)</B>가 크다 — 0/1이 어지간한 잡음·전압 변동에도
          안 뒤집힌다. 디지털이 아날로그보다 견고하고 미세화에 잘 버티는 근본 이유다.
        </Deep>
        <Note label="2부 요약: ">
      ...
      <Section id="c8">
        ...
        />
        <Callout label="사실은 세 번째 메모리, SRAM: ">
          DRAM·NAND 말고 하나 더 있다. CPU 안에 들어가는 <B>SRAM</B>은 트랜지스터 6개로 비트 하나를 잡아(6T) 축전기 없이도
          상태를 유지하므로 <B>가장 빠르지만</B>, 셀이 커서 비싸고 용량이 작다. 그래서 컴퓨터 메모리는 <Em>속도↔용량↔가격</Em>
          을 맞바꾸는 계단(메모리 계층)으로 쌓인다: 레지스터 → <B>SRAM 캐시</B>(CPU 안, ns) → <B>DRAM</B>(메인 메모리, 수십
          ns) → <B>NAND SSD</B>(저장, 수십 µs) → 그 아래로. 위로 갈수록 빠르고 작고 비싸다. "왜 굳이 여러 종류냐"는 답이
          여기 있다 — 하나로는 빠름과 큼을 동시에 못 잡는다.
        </Callout>
      ...
      <Section id="c9">
        ...
        <Deep title="한 걸음 더 · 무어의 법칙과 데나드 스케일링은 다르다">
          흔히 뭉뚱그리지만 두 가지다. <B>무어의 법칙</B>은 "트랜지스터 <Em>수</Em>가 2년마다 2배"라는 경제적 관찰이고, 그
          공짜 보너스를 준 게 <B>데나드 스케일링</B>(1974)이다 — 트랜지스터를 작게 만들면 전압도 같이 낮출 수 있어 <Em>전력
          밀도가 일정</Em>하게 유지된다는 물리 법칙. 덕분에 2000년대 중반까지는 미세화 = 자동으로 더 빠르고 시원한 칩이었다.
          그런데 ~2005년경 전압을 더는 못 낮추게 되면서(누설 때문에) 데나드 스케일링이 깨졌다. 그때부터 "작아져도 더 빨라지진
          않는" 시대가 됐고, 그 출구가 바로 병렬화(17장)다. 또 오늘날 "3nm"는 실제 트랜지스터 치수가 아니라 세대를 가리키는
          마케팅 이름에 가깝다 — 실제 게이트 길이는 그보다 훨씬 크다.
        </Deep>
        <Deep title="학부 수준 · 미세화의 숨은 적, 배선 RC 지연">
          트랜지스터는 빨라졌는데, 이들을 잇는 구리 배선이 만드는 <B>RC 지연</B>(저항 R × 정전용량 C)이 점점 전체 속도를
          지배한다. 선이 가늘고 길수록 R↑, 인접 선과 가까울수록 C↑ — 미세화가 둘 다 악화시킨다(지연 ∝ RC). 그래서 R을 줄이려
          알루미늄→<B>구리</B> 배선으로 바꾸고, C를 줄이려 <B>low-k 절연체</B>를 쓰고, 한 평면에 다 못 넣어 <B>수십 층</B>으로
          쌓는다. "게이트 지연 &lt; 배선 지연"이 된 지 오래라, 현대 설계의 진짜 싸움은 연산이 아니라 <Em>신호를 어떻게 빨리
          옮기느냐</Em>다(17장 메모리 월과 같은 결).
        </Deep>
      ...
      <Section id="c10">
        ...
        <Deep title="한 걸음 더 · 먼지 한 톨이 칩을 죽인다">
          이 모든 공정은 <B>클린룸</B>에서 일어난다. 회로 선폭이 나노미터인데 머리카락은 8만 nm, 담배 연기 입자도 수백 nm다 —
          먼지 한 톨이 회로 위에 떨어지면 그 자리 칩은 그대로 불량이다. 그래서 최첨단 팹의 공기는 일반 사무실보다 입자가
          <B>수만~수십만 배 깨끗</B>하고(ISO 1~3등급), 사람은 방진복으로 온몸을 감싼다. 그런데도 결함은 생기므로 매 층마다
          <Em>검사(7번)</Em>가 들어가는 것이다. 수백 단계 중 한 층만 어긋나도 그 위에 쌓은 모든 게 헛수고가 되니, 일찍
          잡을수록 돈을 아낀다 — 이 "조기 발견의 경제학"이 11장 검사의 존재 이유다.
        </Deep>
        <Deep title="학부 수준 · 노광 해상도의 공식">
          얼마나 작게 그릴 수 있나는 한 식으로 요약된다 — <Kbd>해상도 = k₁·λ / NA</Kbd>. λ=빛의 파장, NA=렌즈 개구수(빛을
          모으는 능력), k₁=공정 상수(이론 하한 ≈ 0.25). 더 작게 그리려면 ① <B>λ↓</B>(193nm → EUV 13.5nm) ② <B>NA↑</B>
          (렌즈를 키움 — High-NA EUV) ③ <B>k₁↓</B>(공정 기교). 그런데 동시에 <B>초점 심도 DOF ∝ λ/NA²</B>는 줄어들어 웨이퍼
          평탄도·정렬이 가혹해진다. EUV 이전엔 193nm로 한계를 넘으려 같은 층을 여러 번 찍는 <B>멀티 패터닝</B>(LELE 등)을
          썼는데 공정 수·비용·오버레이 부담이 컸다 — EUV가 이를 단번에 덜어준 이유다.
        </Deep>
      ...
      <Section id="c11">
        ...
        <Callout label="모든 결함이 똑같지 않다: ">
          검사의 진짜 어려움은 "결함을 찾는 것"이 아니라 <B>"신경 쓸 결함만 골라내는 것"</B>이다. 회로를 실제로 죽이는{' '}
          <B>킬러 결함(killer defect)</B>이 있고, 있어도 동작에 지장 없는 <B>무해 결함(nuisance)</B>이 있다. 무해 결함까지 다
          잡아 멈추면 멀쩡한 웨이퍼를 버리게 되고, 너무 느슨하면 불량을 흘려보낸다. 게다가 웨이퍼 한 장을 1픽셀까지 다 보면
          너무 느려서, 보통은 <Em>대표 지점만 샘플링</Em>해 통계로 공정을 감시한다. 그래서 검사는 광학·물리만의 문제가 아니라
          "무엇을, 얼마나, 어떻게 판정할 것인가"라는 <B>데이터·소프트웨어·의사결정</B> 문제다.
        </Callout>
      ...
      <Section id="c12">
        ...
        <Deep title="한 걸음 더 · 2.5D, 인터포저, TSV (AI 칩의 실제 구조)">
          엔비디아 AI 가속기 같은 첨단 칩은 사실 <B>2.5D 패키징</B>이다. GPU 다이와 HBM 메모리 더미를 <B>인터포저</B>라는 작은
          실리콘 받침판 위에 나란히 올리고, 그 안에 머리카락보다 가는 수천~수만 가닥의 배선을 깔아 초고속으로 잇는다(TSMC의
          <B>CoWoS</B>가 대표). HBM 자체는 진짜 3D로, DRAM 칩들을 위로 쌓고 <B>TSV(실리콘 관통 전극)</B>—칩을 수직으로 뚫는
          전선—로 층층이 연결한다. 즉 "옆으로 빨리(2.5D 인터포저) + 위로 빽빽이(3D TSV)"를 합친 게 오늘날 AI 하드웨어의
          물리적 실체다. 연결 지점이 수만 개라 하나만 틀어져도 불량이고, 그래서 후공정 검사가 결정적이다.
        </Deep>
      ...
      <Section id="c13">
        ...
        <Deep title="한 걸음 더 · 왜 이렇게 잘게 쪼개졌나 (분업의 경제학)">
          최신 팹 하나를 짓는 데 <B>200억 달러</B>가 넘고, EUV 노광기 한 대가 <B>2~3억 달러</B>다. 어느 회사도 설계·제조·장비·
          소재를 다 잘할 수 없으니 극단적으로 분업했다. 설계만 하는 <B>팹리스</B>(엔비디아)는 공장 없이도 최고 칩을 만들고,
          <B>파운드리</B>(TSMC)는 남의 설계를 받아 천문학적 설비로 찍어낸다. 이 구조의 급소가 두 곳 있다 — 노광은 사실상{' '}
          <B>ASML 독점</B>, 최첨단 제조는 사실상 <B>TSMC</B>에 쏠려 있어, 한 곳만 멈춰도 전 세계가 흔들린다. 그래서 반도체가
          경제 안보·지정학의 한복판에 서 있는 것이다.
        </Deep>
      ...
      <Section id="c14">
        ...
        <Callout label="끄는 게 켜는 것보다 어렵다: ">
          물리에는 깨기 힘든 벽이 하나 있다. 상온에서 트랜지스터를 "10배 더 확실히 끄려면" 게이트 전압을 최소{' '}
          <B>약 60mV</B> 더 내려야 한다(서브문턱 스윙, 60mV/decade). 이 한계 때문에 동작 전압을 무한정 낮출 수 없고, 그래서
          데나드 스케일링(9장)이 멈춘 것이다. FinFET·GAA가 게이트로 채널을 더 꽉 감싸는 이유도 결국 이 "끄는 능력"을 60mV
          한계에 최대한 가깝게 끌어올리려는 싸움이다. 이걸 넘으려는 차세대 소자(터널 FET 등)도 연구 중이다.
        </Callout>
        <Deep title="학부 수준 · 누설을 수식으로 (60mV/decade·DIBL·터널링)">
          OFF 상태에서도 전류는 0이 아니라 게이트 전압에 <Em>지수적으로</Em> 변한다(서브문턱 영역). 이 전류를 10배 줄이는 데
          필요한 전압이 <B>서브문턱 스윙</B> <Kbd>S = (kT/q)·ln10 ≈ 60mV/decade</Kbd>(상온 이론 하한)다. 이 벽 때문에 동작
          전압을 무한정 못 낮춘다. 채널이 짧아지면 드레인 전압이 채널의 장벽을 끌어내리는 <B>DIBL</B>(Drain-Induced Barrier
          Lowering)로 V_th가 떨어져 누설이 더 샌다. 게다가 산화막이 원자 몇 층까지 얇아지면 전자가 막을{' '}
          <B>양자 터널링</B>으로 통과해 게이트 누설까지 생긴다 — 그래서 SiO₂ 대신 유전율 높은 <B>high-k(HfO₂)+메탈 게이트</B>로
          바꿔, 물리적으로는 두껍게(터널링↓) 두면서 전기적으로는 얇게(C_ox↑) 만드는 묘수를 썼다(45nm 이후).
        </Deep>
      ...
      <Section id="c15">
        ...
        <Deep title="한 걸음 더 · DRAM의 진짜 어려움은 '축전기'">
          DRAM 미세화의 병목은 트랜지스터가 아니라 <B>축전기</B>다. 셀이 작아져도 전하를 충분히 담으려면 일정 용량이 필요해서,
          좁은 바닥에 깊은 우물을 파듯 <Em>세로로 긴 축전기</Em>를 세운다(가로가 안 되니 세로로). 그래서 DRAM은 NAND처럼
          수백 층으로 막 쌓기가 어렵고, 미세화·재료(high-k 유전체) 싸움이 치열하다. 반면 <B>HBM</B>은 완성된 DRAM 칩 자체를
          TSV로 8~12단 수직 적층해 대역폭을 폭발적으로 늘린 것 — 셀을 줄이는 대신 <Em>칩을 쌓는</Em> 다른 축의 해법이고, 그만큼
          적층·본딩 검사가 수율을 좌우한다.
        </Deep>
      ...
      <Section id="c16">
        ...
        <Callout label="설계도 검증이 곧 소프트웨어 테스트: ">
          사람이 모든 트랜지스터를 그리지 않는 또 다른 비결은 <B>표준 셀(standard cell)</B>이다 — 미리 검증된 게이트·플립플롭
          블록의 라이브러리를 가져다 EDA가 자동으로 배치·배선한다. 그리고 테이프아웃 전에 통과해야 하는 두 관문이 있다:{' '}
          <B>DRC</B>(제조 규칙 위반이 없나, 선 간격 등)와 <B>LVS</B>(그려진 레이아웃이 원래 회로도와 정확히 같나). 둘 다
          본질적으로 거대한 <Em>정적 분석·테스트</Em>다. "칩 설계의 절반이 검증"이라는 말이 과장이 아닌 이유이고, 이 영역이
          소프트웨어 엔지니어링과 가장 많이 겹친다.
        </Callout>
      ...
      <Section id="c17">
        ...
        <Deep title="학부 수준 · 전력 밀도와 DVFS">
          파워 월의 본질은 <B>전력 밀도(W/cm²)</B>다. 동적 전력 <Kbd>P = C·V²·f</Kbd>에서, 데나드 스케일링이 살아있을 땐
          미세화로 C와 V를 같이 낮춰 f를 올려도 단위면적 전력이 일정했다. 전압을 더 못 낮추게 되자(누설 때문에) 같은 면적에서
          전력이 치솟아 핫플레이트 수준이 됐고, 그래서 클럭이 ~4GHz에서 멈췄다. 해법은 영리한 전력 관리다 — <B>DVFS</B>(동적
          전압·주파수 조절)로 부하에 따라 V·f를 실시간 조절하고, 안 쓰는 블록은 전원을 끊는다(<B>파워 게이팅</B>). 설계 목표가
          "최대 클럭"에서 <Em>"와트당 성능(perf/watt)"</Em>으로 바뀐 게 이 시대의 핵심이다.
        </Deep>
        ...
        <Deep title="한 걸음 더 · 병렬화의 한계와 다크 실리콘">
          "코어를 많이"가 만능은 아니다. <B>암달의 법칙</B>: 프로그램에 순차적으로만 돌아가는 부분이 단 10%만 있어도, 코어를
          무한히 늘려도 최대 10배밖에 못 빨라진다. 그래서 병렬화가 잘 먹히는 작업(그래픽·딥러닝의 행렬곱)과 아닌 작업이
          갈린다. 또 하나, <B>다크 실리콘(dark silicon)</B> — 트랜지스터는 많이 넣었지만 전부 동시에 켜면 너무 뜨거워서, 칩의
          상당 부분을 <Em>꺼둔 채</Em> 써야 한다. 그래서 요즘 칩은 범용 코어를 무작정 늘리기보다, 특정 작업에 특화된 가속기
          (GPU·NPU·코덱 등)를 잔뜩 박아 그때그때 필요한 블록만 켜는 방향으로 간다.
        </Deep>
