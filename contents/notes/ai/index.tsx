  ...
  return (
    <NoteRoot accent="var(--brand-violet)">
      ...
      <Section id="c2">
        ...
        <Deep title="한 걸음 더 · '특징'을 누가 고르나 (딥러닝의 진짜 도약)">
          고전 머신러닝의 진짜 일은 모델이 아니라 <B>특징 공학(feature engineering)</B>이었다. "집값"이라면 평수·층수·역까지
          거리 같은 <Em>입력 항목을 사람이 손으로 골라</Em> 넣어야 했고, 이미지라면 "모서리 개수" 같은 특징을 사람이 설계했다.
          모델 성능의 8할이 이 특징 선택에서 갈렸다. 딥러닝의 혁명은 바로 이 단계를 없앤 것이다 — 원시 데이터(픽셀·글자)를
          그대로 넣으면 <B>어떤 특징이 중요한지까지 신경망이 스스로 학습</B>한다(표현 학습, representation learning). 8장 CNN이
          "선→부품→사물"을 스스로 찾는 게 정확히 이것이다. 그래서 데이터만 충분하면 사람의 도메인 지식 없이도 통한다.
        </Deep>
        <Note label="핵심 한 줄: ">
      ...
      <Section id="c5">
        ...
        <Callout label="왜 '깊게' 쌓나 — 보편 근사 정리: ">
          수학적으로는 <B>은닉층 하나만 있어도</B> 뉴런을 충분히 넓게 두면 어떤 연속 함수든 원하는 정밀도로 흉내 낼 수 있다
          (보편 근사 정리). 그런데 왜 굳이 깊게 쌓을까? <Em>효율</Em> 때문이다. 얕고 넓은 망은 같은 문제를 풀려면 뉴런이
          기하급수적으로 많이 필요하지만, 깊은 망은 "단순한 특징을 조합해 복잡한 특징을 만드는" 계층 구조를 쓰므로 훨씬 적은
          파라미터로 같은 일을 해낸다. 세상의 데이터(이미지·언어)가 원래 계층적이라, 그 구조를 닮은 깊은 망이 잘 맞는 것이다.
        </Callout>
        <Deep title="석사 수준 · 깊은 망을 실제로 학습시키는 법">
          단순히 층을 쌓으면 기울기가 뒤로 갈수록 0이 되거나 폭발한다(<B>기울기 소실/폭발</B>). 이를 푸는 네 가지가 현대
          딥러닝의 토대다. ① <B>가중치 초기화</B> — 분산을 층 크기에 맞춰 잡는 Xavier/He 초기화. ② <B>활성화</B> — sigmoid
          대신 ReLU 계열로 기울기 흐름 유지. ③ <B>정규화</B> — 배치 정규화(BatchNorm)·층 정규화(LayerNorm)로 각 층 입력
          분포를 안정화. ④ <B>잔차 연결(residual)</B> — 입력을 출력에 그대로 더해(skip) 기울기가 지름길로 흐르게(ResNet의
          핵심). 이 넷이 없으면 수십 층 이상은 사실상 학습이 안 된다. 트랜스포머도 <Em>LayerNorm + 잔차</Em>로 깊이를 버틴다.
        </Deep>
      ...
      <Section id="c6">
        ...
        <Deep title="석사 수준 · 역전파와 옵티마이저를 수식으로">
          역전파는 <B>연쇄법칙(chain rule)</B>으로 손실 L의 각 가중치 기울기 <Kbd>∂L/∂w</Kbd>를 출력층→입력층으로 거꾸로
          곱해 내려가며 계산한다(계산 그래프의 자동 미분). 분류 손실은 보통 <B>교차 엔트로피</B> <Kbd>L = −Σ y·log(ŷ)</Kbd>.
          기울기를 구한 뒤 갱신 방식이 옵티마이저다: ① <B>SGD</B>: <Kbd>w ← w − η·g</Kbd>. ② <B>모멘텀</B>: 과거 기울기의
          관성을 더해 진동을 줄이고 골을 빨리 내려감. ③ <B>Adam</B>: 기울기의 1차(평균)·2차(분산) 모멘트를 추정해{' '}
          <Em>파라미터마다 학습률을 자동 조절</Em> — 사실상 표준. 학습률 η는 보통 워밍업 후 코사인 감쇠 스케줄을 쓴다.
        </Deep>
      ...
      <Section id="c7">
        ...
        <Deep title="한 걸음 더 · 편향–분산 트레이드오프">
          과소적합과 과적합은 사실 한 저울의 양 끝이다. <B>편향(bias)</B>이 높다 = 모델이 너무 단순해 패턴 자체를 못 잡음
          (과소적합). <B>분산(variance)</B>이 높다 = 모델이 데이터마다 너무 출렁여 잡음까지 외움(과적합). 모델을 키우면 편향은
          줄지만 분산이 늘고, 줄이면 그 반대 — 그 사이 <Em>달콤한 지점</Em>을 찾는 게 전통적 관점이다. (흥미롭게도 아주 거대한
          신경망에서는 이 곡선이 한 번 더 꺾여 더 키울수록 다시 좋아지는 "이중 강하" 현상이 관찰되는데, 그래서 현대 LLM은
          "그냥 더 크게"가 통하는 면이 있다.)
        </Deep>
        <Deep title="석사 수준 · 정규화를 수식으로">
          과적합을 누르는 정공법들: ① <B>L2 정규화(가중치 감쇠)</B> — 손실에 <Kbd>λ·Σw²</Kbd>를 더해 가중치가 커지지 않게
          한다(베이즈적으로는 가중치에 가우시안 사전분포를 두는 것과 동치). ② <B>드롭아웃</B> — 매 스텝 뉴런을 확률 p로 끄면,
          사실상 지수적으로 많은 부분 망의 <Em>앙상블</Em>을 학습하는 효과. ③ <B>조기 종료·데이터 증강·라벨 스무딩</B>. 일반화
          이론에선 모델의 '유효 복잡도'를 줄이는 것으로 본다. 흥미롭게도 현대 거대 모델은 데이터가 워낙 많아 명시적 정규화보다{' '}
          <B>데이터 규모·다양성 자체</B>가 가장 강력한 정규화로 작동하기도 한다.
        </Deep>
        <Note label="2부 요약: ">
      ...
      <Section id="c9">
        ...
        <Deep title="한 걸음 더 · 공짜는 아니다 (어텐션의 n² 비용)">
          모든 단어가 모든 단어를 본다는 건, 단어가 n개면 비교가 <B>n×n번</B> 일어난다는 뜻이다. 그래서 입력이 2배 길어지면
          연산·메모리는 <Em>약 4배</Em>로 는다. 이 제곱 비용이 "컨텍스트 윈도우"가 왜 비싸고 한계가 있는지의 근본 이유다(12장).
          그래서 긴 문맥을 싸게 처리하려는 연구(희소 어텐션, 플래시어텐션, 상태공간 모델 등)가 활발하다 — 어텐션의 표현력은
          지키면서 n² 벽을 깎아내려는 싸움이다.
        </Deep>
      </Section>
      ...
      <Section id="c10">
        ...
        <Callout label="순서는 어떻게 알까 — 위치 인코딩: ">
          한 가지 역설이 있다. 어텐션은 모든 단어를 "동시에" 보기 때문에 그 자체로는 <B>단어의 순서를 모른다</B> — "개가 사람을
          물다"와 "사람이 개를 물다"가 똑같이 보인다. 그래서 트랜스포머는 각 단어 벡터에 <B>위치 정보(positional encoding)</B>를
          더해 "너는 3번째 단어야"를 표시한다. RNN은 한 단어씩 읽으니 순서가 공짜였지만, 병렬을 얻은 대가로 순서를 따로
          주입해야 하는 것이다. 최신 모델이 긴 문맥을 다루는 비결(RoPE 등)도 대부분 이 위치 인코딩을 영리하게 만드는 데 있다.
        </Callout>
        <Deep title="석사 수준 · 어텐션을 수식으로">
          각 토큰을 Query·Key·Value 벡터로 사영한 뒤, <Kbd>Attention = softmax(Q·Kᵀ / √d_k)·V</Kbd>. 직관: Q와 K의 내적이
          "관련도 점수"이고, <B>√d_k로 나눠</B> 점수 분산을 안정화하며(스케일드 닷프로덕트), softmax로 가중치를 만들어 V를
          가중합한다. <B>멀티헤드</B>는 이를 h개 병렬로 해 서로 다른 부분공간의 관계를 잡고 이어붙인다. 생성 모델(GPT)은
          미래 토큰을 못 보게 막는 <B>인과(causal) 마스킹</B>을 쓴다. 각 트랜스포머 블록은 (멀티헤드 어텐션 → 피드포워드 MLP)
          에 부분마다 <Em>잔차 연결 + LayerNorm</Em>을 두른 구조이고, 이걸 수십~수백 겹 쌓은 게 LLM이다.
        </Deep>
      ...
      <Section id="c11">
        ...
        <Deep title="한 걸음 더 · 토큰화와 스케일링 법칙">
          모델은 글자나 단어가 아니라 <B>토큰</B>(자주 쓰는 글자 덩어리) 단위로 본다. 영어는 대략 한 단어가 1~2토큰, 한국어는
          더 잘게 쪼개진다. "strawberry의 r 개수"를 모델이 곧잘 틀리는 것도 글자가 아니라 토큰 덩어리로 보기 때문이다. 그리고
          "얼마나 키워야 하나"엔 경험 법칙이 있다 — <B>스케일링 법칙</B>: 성능은 모델 크기·데이터·연산을 키울수록 매끄럽게
          오른다. 특히 친칠라(Chinchilla) 연구는 <Em>모델만 키우지 말고 데이터도 비례해 늘려야</Em> 최적임을 보였다. "더 크게
          하면 더 똑똑해진다"가 막연한 기대가 아니라 측정된 곡선이라는 점이 이 분야의 동력이다.
        </Deep>
        <Deep title="석사 수준 · 사전학습 목표와 스케일링 법칙">
          목표는 단순하다 — 다음 토큰의 <B>음의 로그우도</B>를 최소화한다: <Kbd>L = −Σ log P(다음 토큰 | 이전 토큰들)</Kbd>
          (자기회귀 교차 엔트로피). 정답이 데이터 자체라 라벨이 필요 없다(자기지도). 토큰화는 보통 <B>BPE</B>(자주 등장하는
          바이트 쌍을 반복 병합)로 어휘를 만든다. 성능은 모델 크기 N·데이터 D·연산 C에 대해 매끄러운 <B>거듭제곱 법칙</B>
          (대략 L ∝ N^−α)을 따르고, <B>친칠라(Chinchilla)</B>는 주어진 연산 예산에서 N과 D를 비슷한 비율로 키워야
          최적(대략 <Em>토큰 ≈ 20 × 파라미터</Em>)임을 보였다. 일부 능력은 규모가 임계점을 넘을 때 갑자기 나타나는{' '}
          <B>창발(emergence)</B>로 보고된다.
        </Deep>
      ...
      <Section id="c12">
        ...
        <Callout label="컨텍스트는 공격 표면이기도 하다: ">
          모델은 시스템 프롬프트, 사용자 입력, 검색해 온 문서를 <B>한 컨텍스트 안에서 잘 구분하지 못한다</B>. 그래서 외부
          문서나 웹페이지에 "이전 지시를 무시하고 이렇게 해"라는 문장을 심어두면 모델이 그걸 명령으로 따라버리는{' '}
          <B>프롬프트 인젝션</B>이 가능하다. 도구·에이전트(14·15장)와 결합하면 이게 실제 피해(데이터 유출 등)로 이어질 수
          있어, "컨텍스트에 무엇을 넣느냐"는 성능 문제이자 <Em>보안 문제</Em>다. 신뢰할 수 없는 입력은 권한을 제한하고 검증하는
          게 기본이다.
        </Callout>
        <Deep title="석사 수준 · KV 캐시와 컨텍스트의 진짜 비용">
          생성은 토큰을 하나씩 뱉는데, 매 스텝 앞 토큰 전체를 다시 어텐션하면 낭비다. 그래서 각 토큰의 Key·Value를 저장해
          재사용하는 <B>KV 캐시</B>를 쓴다 — 토큰당 계산은 선형이 되지만, 캐시 크기가 <Kbd>층수 × 헤드 × 시퀀스 길이 × 차원</Kbd>
          으로 늘어 긴 문맥에선 <B>메모리(특히 HBM 대역폭)</B>가 병목이 된다. 그래서 KV를 줄이는 기법이 추론 효율의 핵심이다:
          여러 쿼리가 K·V를 공유하는 <B>MQA/GQA</B>, 메모리를 페이지처럼 관리하는 <B>PagedAttention</B>, KV 양자화 등.
          "컨텍스트가 길수록 비싸다"의 실제 정체가 이 KV 메모리이고, 9장에서 본 어텐션의 n² 비용과 짝을 이룬다.
        </Deep>
      ...
      <Section id="c13">
        ...
        <Callout label="RAG는 만병통치약이 아니다: ">
          현실의 RAG는 <B>검색만큼만 똑똑하다</B>(garbage in, garbage out). 흔한 실패 모드: ① 관련 문서를 못 찾으면 모델은
          여전히 지어낸다, ② 엉뚱한 문서를 가져오면 그걸 "근거"로 자신 있게 틀린다, ③ 청크가 너무 잘게 잘려 문맥이 끊기거나
          너무 크면 핵심이 묻힌다, ④ 검색은 잘했는데 모델이 그 근거를 무시하기도 한다. 그래서 RAG 품질 작업의 대부분은 LLM이
          아니라 <Em>검색·청킹·리랭킹·평가</Em>를 다듬는 데 든다. "출처를 댄다"가 곧 "맞다"는 아니므로, 답이 실제로 근거에서
          왔는지 확인하는 장치(인용 검증)가 중요하다.
        </Callout>
      ...
      <Section id="c14">
        ...
        <Deep title="한 걸음 더 · 도구는 양날의 검 (구조적 출력과 권한)">
          도구 사용이 강력한 만큼 위험도 따라온다. 모델이 <Em>엉뚱한 도구를 고르거나, 인자를 틀리게 채우거나, 위험한 작업
          (파일 삭제·결제)을 실행</Em>할 수 있다. 그래서 실무에선 ① 도구 호출을 반드시 정해진 형식(JSON 스키마)으로만 받게
          강제하고(구조적 출력), ② 각 도구에 <B>권한·한도</B>를 두고(읽기 전용/승인 필요), ③ 되돌릴 수 없는 작업은 사람 확인을
          거치게 한다. "모델에게 손발을 달아준다"는 건 곧 "그 손발을 통제하는 골격이 필요하다"는 뜻이고, 그게 다음 장의
          하네스다.
        </Deep>
        <Note label="의미: ">
      ...
      <Section id="c16">
        ...
        <Deep title="한 걸음 더 · 추론 모델은 어떻게 '생각'을 배우나">
          비결은 <B>검증 가능한 보상으로 하는 강화학습</B>이다. 수학·코딩처럼 정답을 자동으로 채점할 수 있는 문제를 잔뜩 주고,
          모델이 길게 풀이를 쓰게 한 뒤 <Em>정답을 맞힌 풀이를 강화</Em>한다. 그러면 모델은 "어떻게 끄적여야 정답에 닿는지"를
          스스로 터득한다 — 사람이 풀이 과정을 일일이 가르치지 않아도, 되돌아가기·검산 같은 행동이 창발한다(DeepSeek R1이 보인
          것). 다만 한계도 분명하다: 정답을 자동 채점하기 어려운 열린 문제(글쓰기·판단)에는 이 방식이 잘 안 통하고, 길게
          생각할수록 토큰·시간·비용이 그대로 늘어난다. "항상 추론 모델"이 답이 아니라, 어려운 문제에만 골라 쓰는 게 실무다.
        </Deep>
        <Deep title="석사 수준 · 정렬과 강화학습 변종 (RLHF·DPO·GRPO)">
          <B>RLHF</B> 고전 파이프라인: ① 사람 선호 쌍으로 <B>보상 모델 R</B>을 학습 → ② 정책(LLM)을 <B>PPO</B>로 R을
          최대화하되, 원래 모델에서 너무 벗어나지 않게 <B>KL 페널티</B>를 건다(보상 해킹 방지). 최근엔 보상 모델 없이 선호
          데이터로 정책을 바로 최적화하는 <B>DPO</B>, 가치함수 없이 그룹 표본의 상대 보상으로 학습하는 <B>GRPO</B>(추론 모델
          학습에 인기)가 부상했다. 추론 특화에선 결과만 채점하는 ORM 대신 <Em>풀이 단계마다 채점</Em>하는 과정 보상
          모델(PRM)도 쓴다. 핵심 통찰: <B>"무엇을 보상으로 정의하느냐"가 곧 모델의 행동을 결정</B>한다 — 검증 가능한
          보상(수학·코드 정답)이 추론 능력을 끌어낸 이유다.
        </Deep>
      ...
      <Section id="c17">
        ...
        <Deep title="한 걸음 더 · 이제는 '학습'보다 '추론' 비용">
          한 번 학습한 모델은 수억 명이 매일 쓴다. 그래서 누적 비용은 점점 <B>추론(inference)</B> 쪽이 지배한다 — 모델을 한 번
          만드는 값보다, 매 질문에 답하느라 GPU를 돌리는 값이 훨씬 크다. 그래서 효율화의 초점도 추론으로 옮겨갔다: <B>양자화</B>
          (FP16→FP8→INT4로 정밀도를 낮춰 메모리·속도 확보), <B>KV 캐시</B>(이미 처리한 토큰의 계산을 재사용), 배칭, MoE로
          활성 파라미터 줄이기. "성능을 지키며 한 토큰당 비용을 깎는" 이 싸움이 곧 AI 서비스의 수익성과 직결된다.
        </Deep>
        <Deep title="석사 수준 · MoE·양자화·추측 디코딩의 실제">
          <B>MoE</B>: 게이팅 네트워크가 각 토큰을 점수 매겨 상위 k개 전문가에게만 보낸다(top-k 라우팅). 전문가가 한쪽에
          쏠리지 않게 <B>부하 분산 보조 손실</B>을 더하고 전문가별 용량(capacity)을 둔다 — 그래서 총 파라미터는 크되 토큰당
          활성 파라미터는 작다. <B>양자화</B>: 가중치를 FP16→INT8/INT4로 줄이되 정확도를 지키려 2차 정보 기반 GPTQ, 활성값을
          고려하는 AWQ 같은 보정 기법을 쓰고, 학습·추론에 FP8도 확산 중. <B>증류</B>: 학생 모델이 교사의 출력 분포를 KL
          발산으로 모사. 그리고 작은 모델이 초안을 쓰고 큰 모델이 한 번에 검증하는 <B>추측 디코딩(speculative decoding)</B>으로
          품질을 지키며 속도를 끌어올린다. 전부 <Em>같은 와트로 더 많은 지능</Em>을 짜내려는 기법이다.
        </Deep>
        <Note label="4부 요약: ">
      ...
      <Section id="c18">
        ...
        <Callout label="규모 감각 — 전력이 곧 한계: ">
          AI의 병목은 점점 칩이 아니라 <B>전기와 열</B>이다. 최신 AI 가속기 한 장이 700W~1kW를 먹고, 데이터센터는 수만 장을
          묶어 돌리니 한 곳이 도시 하나급 전력을 쓴다. 그래서 빅테크가 발전소·원전 계약까지 맺는 시대가 됐고, "토큰당 전력"이
          모델 효율의 진짜 지표가 됐다. 17장의 효율화(양자화·MoE)와 18장의 하드웨어(HBM·전용칩)가 모두 결국 <Em>같은 와트로 더
          많은 지능을</Em> 짜내려는 싸움이다.
        </Callout>
        <Note label="교차점: ">
      ...
      <Section id="c19">
        ...
        <Deep title="한 걸음 더 · 정렬(alignment)과 평가의 어려움">
          모델을 "쓸 만하고 안전하게" 만드는 정렬은 끝난 문제가 아니다. 정교하게 프롬프트를 짜 안전장치를 우회하는{' '}
          <B>탈옥(jailbreak)</B>, 보상을 엉뚱하게 최적화하는 <B>보상 해킹</B>, 그럴듯하게 사람을 설득하지만 틀린 답 같은 게
          계속 나온다. 게다가 모델이 워낙 잘해져서 <Em>평가(evaluation)</Em> 자체가 어렵다 — 벤치마크를 학습 데이터에서 이미
          본 "오염" 문제, 채점자(사람·AI)를 속이는 문제 등. 그래서 "이 모델이 정말 더 나은가"를 재는 일이 연구의 한 분야가
          됐다. 능력이 오를수록, <B>능력을 검증하고 통제하는 기술</B>의 중요성도 같이 커진다.
        </Deep>
